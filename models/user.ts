...
export class User
{
    ...
    public async pull(recursive = true): Promise<boolean>
    {
        ...
        const {data,error} = await supabase.from("profiles").select("*").eq("id",this.m_data.id).single();
        if (error)
        {
            return false;
        }
    ...
    public async pullLeaveRequests(): Promise<boolean>
    {
        ...
        const requests = await supabase.from("leave_requests").select("id").eq("user_id",this.m_data.id);
        if (requests.error)
        {
            return false;
        }
    ...
    {
        const supabase = createClient();
        const response = await supabase.from("profiles").update(this.m_data).eq("id",this.m_data.id);
        return (!response.error);
    }

    public get(): UserData
    {
        this.m_data.leave_requests = this.m_leave_requests.map(request => request.get());
        return this.m_data;
    }
